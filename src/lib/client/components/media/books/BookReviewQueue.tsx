...
export function BookReviewQueue({selectedIds, onReview, busy}: {selectedIds: number[]; onReview: (ids: number[]) => void; busy: boolean}) {
    ...
    const [confidence, setConfidence] = useState<"all" | "high" | "possible">("all");
    ...
    return <Card className="min-h-0 min-w-0">
        <CardHeader><div className="flex items-start justify-between gap-3"><div className="flex flex-col gap-2"><CardTitle>Merge review</CardTitle>
            ...
            <ToggleGroup aria-label="Match confidence" variant="outline" size="sm" spacing={0} value={[confidence]} onValueChange={values => {
                ...
                <ToggleGroupItem value="high" className="flex-1">High confidence {queue.data?.highConfidence ?? "…"}</ToggleGroupItem>
                <ToggleGroupItem value="possible" className="flex-1">Possible {queue.data?.possible ?? "…"}</ToggleGroupItem>
            </ToggleGroup>
        ...
        <CardContent className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto overscroll-contain">
            ...
            {groups.map(group => <article key={group.key} className={cn("flex flex-col gap-3 rounded-lg border p-4", group.key === selectedKey && "border-primary/40 bg-primary/5")}>
                <div className="flex flex-wrap items-center justify-between gap-2"><Badge variant={group.confidence === "high" ? "secondary" : "outline"}>{group.confidence === "high" ? "High confidence" : "Possible match"}</Badge>
                    <span className="text-xs tabular-nums text-muted-foreground">{group.works.length} works · {group.readers} list entries</span>
                </div>
                <ul className="flex flex-col gap-2">{group.works.map(work => <li key={work.id} className="flex items-center gap-3">
                    ...
                    <span className="shrink-0 text-xs tabular-nums text-muted-foreground">{work.readers} readers</span>
                </li>)}</ul>
            ...
            {queue.data && groups.length === 0 && <Empty><EmptyHeader><EmptyMedia variant="icon"><ScanSearch/></EmptyMedia><EmptyTitle>No pending matches</EmptyTitle>
                <EmptyDescription>{confidence === "all" ? "Run a scan to look for matching works in your catalogue. Maintenance also refreshes this queue." : "Try another confidence filter, or scan your catalogue again."}</EmptyDescription>
            </EmptyHeader></Empty>}
        </CardContent>
