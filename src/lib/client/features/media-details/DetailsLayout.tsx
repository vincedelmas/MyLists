        ...
        <PageTitle title={media.name} onlyHelmet>
            ...
            <div className="grid grid-cols-12 gap-8 mx-auto px-4 py-2 max-sm:py-0 max-lg:grid-cols-1">
                <div className="col-span-8 space-y-8 max-lg:col-span-1 max-lg:order-2">
                    ...
                    <MediaSynopsis media={media}/>

                    {extraSections}

                    <SimilarMedia mediaType={kind} similarMedia={similarMedia}/>

                    <DetailsExtraSection title="Community activity">
                        <MediaCommunityActivity mediaId={media.id} mediaType={kind}/>
                    </DetailsExtraSection>

                    <DetailsExtraSection title="Community collections">
                        <MediaCommunityCollections mediaId={media.id} mediaType={kind}/>
                    </DetailsExtraSection>
                </div>
                <div className="col-span-4 space-y-6 max-lg:col-span-1 max-lg:order-1">
                    <div className="space-y-6 max-lg:grid max-lg:grid-cols-2 max-md:grid-cols-1 max-lg:gap-6">
                        <div className="space-y-6 max-lg:mb-0">
                            {!isAnonymous &&
                                <RefreshAndEdit mediaId={media.id} mediaType={kind} lastUpdate={media.lastApiUpdate}/>
                            }
                            ...
                            {userMedia && isMediaTypeActive ?
                                <UserMediaDetails userMedia={userMedia} queryOption={queryOption} progressMetadata={progressMetadata}/>
                                :
                                isAnonymous ?
                                    <LockedContent
                                        variant="inline"
                                        ...
                                        description="Log-in or register to add this media to your list, track your progress, add ratings, comments, tags and more."
                                    />
                                    :
                                    !isMediaTypeActive ?
                                        <DisabledMediaListNotice mediaType={kind}/>
                                        :
                                        <Card>
                            ...
                            <CollectionsLists mediaId={media.id} mediaType={kind} isAnonymous={isAnonymous}/>
                        </div>

                        <MediaFollowsSection isAnonymous={isAnonymous} followsData={followsData}/>
                    </div>
                </div>
            </div>
        </PageTitle>
