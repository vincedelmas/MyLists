...
export async function readProviderError(provider: string, response: Response, retryAfterMs?: number, quotaResetAt?: number) {
    const parsed = errorBodySchema.safeParse(await response.json().catch(() => null));
    const error = parsed.success && typeof parsed.data.error === "object" ? parsed.data.error : undefined;
    ...
    if (/dailyLimitExceeded/i.test(reason ?? "") || /perday/i.test(quotaLimit ?? "")) {
        ...
        kind = "item";
        message = statusCode === 410 ? "Media no longer available on the API." : `Unexpected Error: ${statusCode}`;
    }
