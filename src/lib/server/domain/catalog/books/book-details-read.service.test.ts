...
const { BookLibraryRepository } = await import("@/lib/server/domain/library/books/book-library.repository");
const { BookLibraryService } = await import("@/lib/server/domain/library/books/book-library.service");
const { BookDetailsReadService } = await import("./book-details-read.service");
const { BookCatalogReadRepository } = await import("./book-catalog-read.repository");


describe("v2 book detail reads", () => {
    let sqlite: Database;
    ...
    beforeEach(async () => {
        ...
        await db.insert(schema.catalogItem).values([
            ...
            { id: 1001, kind: MediaType.BOOKS, primaryProvider: "google-books", primaryExternalId: "volume-778", name: "Sequel", imageCover: "sequel.jpg", releaseDate: "2026-01-01" },
        ]);
    ...
    it("returns catalog, viewer, follow, author-job and similar-book contracts", async () => {
        const reader = new BookDetailsReadService();
        ...
        expect(result).toMatchObject({
            ...
            userMedia: expect.objectContaining({ mediaId: 1000, actualPage: 120, redo: 0, total: 120, rating: 9 }),
            followsData: [expect.objectContaining({ id: 43, userMedia: expect.objectContaining({ actualPage: 300, redo: 2, total: 900 }) })],
            similarMedia: [expect.objectContaining({ mediaId: 1001, mediaName: "Sequel" })],
        });
        expect(await reader.getMediaJobDetails(JobType.CREATOR, "Author", { page: 1, perPage: 24 }, 42)).toMatchObject({
            total: 2,
