...
const { GameLibraryReadRepository } = await import("@/lib/server/domain/library/games/game-library-read.repository");
...
describe("game details query", () => {
    ...
    it("returns the complete catalog, viewer, follow, collection and compatible-platform contracts", async () => {
        const query = new GameDetailsQuery();
        ...
        const catalog = new GameCatalogReadRepository();
        expect(await catalog.getCompatiblePlatforms(1000)).toEqual([{ name: "PC" }]);
        expect(await catalog.getMediaJobDetails(JobType.CREATOR, "Studio", 0, 24, 42)).toMatchObject({
            total: 1,
