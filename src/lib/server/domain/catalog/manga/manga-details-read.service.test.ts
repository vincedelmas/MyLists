...
const { MangaLibraryService } = await import("@/lib/server/domain/library/manga/manga-library.service");
const { MangaDetailsReadService } = await import("./manga-details-read.service");


describe("v2 manga detail reads", () => {
    let sqlite: Database;
    ...
    it("returns catalog, viewer, follow, author-job and similar-manga contracts", async () => {
        const reader = new MangaDetailsReadService();
        ...
        expect(result).toMatchObject({
            ...
            userMedia: expect.objectContaining({ mediaId: 1000, currentChapter: 120, redo: 0, total: 120, rating: 9 }),
            followsData: [expect.objectContaining({ id: 43, userMedia: expect.objectContaining({ currentChapter: 300, redo: 2, total: 900 }) })],
        ...
        expect(await reader.getMediaJobDetails(JobType.PUBLISHER, "Publisher", { page: 1, perPage: 24 }, 42)).toMatchObject({
            total: 2,
            items: expect.arrayContaining([expect.objectContaining({ mediaId: 1000, inUserList: true })]),
        });
    });
