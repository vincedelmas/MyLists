...
const { MovieDetailsReadService } = await import("./movie-details-read.service");
const { MovieCatalogReadRepository } = await import("./movie-catalog-read.repository");
const { MovieLibraryReadRepository } = await import("@/lib/server/domain/library/movies/movie-library-read.repository");
...
describe("v2 movie details read boundary", () => {
    let sqlite: Database;
    ...
    it("serves movie metadata, collection, private state and followed state with legacy IDs", async () => {
        ...
        await library.replaceRewatches({ userId: 42, catalogItemId: 1000, redo: 2 });
        ...
        const result = await new MovieDetailsReadService().getMediaAndUserDetails(42, 1000);
        ...
        expect(result?.followsData).toEqual([
            expect.objectContaining({
                id: 50,
                name: "followed-owner",
                userMedia: expect.objectContaining({ mediaId: 1000, status: Status.COMPLETED, total: 1 }),
            }),
        ]);
    ...
    it("updates manager-editable movie fields while repairing duration stats", async () => {
        const library = new MovieLibraryService(new MovieLibraryRepository());
        await library.add({ userId: 42, catalogItemId: 1000, status: Status.COMPLETED });
        const admin = new MovieCatalogAdminRepository();
