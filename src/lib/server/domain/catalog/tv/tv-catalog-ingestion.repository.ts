...
/**
 * Adapter from the retained TMDB provider/transformer contract to the canonical
 * TV catalog.
 */
export class TvCatalogIngestionRepository implements MediaIngestionRepository<UpsertTvWithDetails> {
    ...
    async findByApiId(apiId: number | string) {
        return getDbClient()
            .select({ id: catalogItem.id, apiId: catalogItem.primaryExternalId })
            .from(catalogItem)
            .where(and(
    ...
    private async syncNamedRows(
        catalogItemId: number,
        table: typeof tvActor | typeof tvNetwork,
        ...
        if (mode === "refresh") {
            await getDbClient().delete(table).where(eq(table.catalogItemId, catalogItemId));
        }
        await getDbClient()
            .insert(table)
    ...
    private async syncSeasons(
        catalogItemId: number,
        rows: { season: number; episodes: number }[] | undefined,
        mode: "store" | "refresh",
    ) {
...
const uniqueNames = (rows?: { name: string }[]) => [...new Set(
    (rows ?? []).map(({ name }) => name.trim()).filter(Boolean),
)];
