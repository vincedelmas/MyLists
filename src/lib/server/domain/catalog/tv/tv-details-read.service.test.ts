...
const { TvDetailsReadService } = await import("./tv-details-read.service");
...
const { TvLibraryReadRepository } = await import("@/lib/server/domain/library/tv/tv-library-read.repository");
...
describe("v2 TV details read boundary", () => {
    let sqlite: Database;
    ...
    it("serves catalog, private viewer state and followed state with catalog IDs", async () => {
        ...
        const library = new TvLibraryService(libraryRepository);
        ...
        const result = await new TvDetailsReadService(MediaType.SERIES).getMediaAndUserDetails(42, 1000);
        ...
        expect(result?.media).toMatchObject({
            ...
            epsPerSeason: [{ season: 1, episodes: 10 }, { season: 2, episodes: 8 }],
        });
        ...
        expect(result?.followsData).toEqual([
            expect.objectContaining({
                id: 50,
                name: "followed-owner",
                userMedia: expect.objectContaining({ mediaId: 1000, status: Status.COMPLETED, total: 18 }),
            }),
        ]);
    ...
    it("does not expose viewer library state to an anonymous detail request", async () => {
        ...
        const result = await new TvDetailsReadService(MediaType.SERIES).getMediaAndUserDetails(undefined, 1000);
