...
/** Projects movie watch activity into the activity editor contract. */
export class MovieActivityReadRepository {
    async getStatsActivities(access: MediaListAccessScope, timeBucket: string) {
        return getDbClient()
            ...
            .where(and(
                ...
                eq(libraryActivity.kind, MediaType.MOVIES),
    ...
    async getPaginatedActivities(access: MediaListAccessScope, filters: PaginatedActivityFilter) {
        ...
        if (filters.mediaIdsByType) {
            const matchingIds = filters.mediaIdsByType[MediaType.MOVIES] ?? [];
    ...
    async getActivityStatsByMonth(filters: { userId?: number; startMonth: string; excludeBulkImports?: boolean }) {
        const conditions: SQL[] = [
            eq(libraryActivity.kind, MediaType.MOVIES),
            eq(libraryActivity.hidden, false),
            gt(libraryActivity.unitsGained, 0),
        ...
        if (filters.excludeBulkImports) {
            const bulkActivity = alias(libraryActivity, "movie_bulk_activity");
            const likelyBulkMonths = getDbClient()
                .select({ userId: bulkActivity.userId, monthBucket: bulkActivity.monthBucket })
                .from(bulkActivity)
