...
describe("v2 movie library service", () => {
    let sqlite: Database;
    ...
    it("keeps status, watch count, stats, activity and history in one movie transition", async () => {
        ...
        const rewatched = await library.replaceRewatches({ userId: 42, catalogItemId: 1000, redo: 2 });
    ...
    it("serves normalized movie activity and advanced statistics through explicit scopes", async () => {
        ...
        await library.replaceRewatches({ userId: 42, catalogItemId: 1000, redo: 2, loggedAt: "2026-06-02" });
