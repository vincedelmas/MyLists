...
describe("v2 TV library service", () => {
    let sqlite: Database;
    ...
    beforeEach(async () => {
        ...
        service = new TvLibraryService(new TvLibraryRepository());
        await seedTv(db);
    });
    ...
    it("keeps progress, rewatches, stats, activity and history in one TV transition model", async () => {
        ...
        expect(history.map(({ id, mediaId, mediaName, payload }) => ({ id, mediaId, mediaName, payload }))).toEqual([
            { id: 4, mediaId: 1000, mediaName: "A difficult show", payload: { old_value: Status.WATCHING, new_value: Status.COMPLETED } },
            { id: 3, mediaId: 1000, mediaName: "A difficult show", payload: { old_value: 0, new_value: 1 } },
            { id: 2, mediaId: 1000, mediaName: "A difficult show", payload: { old_value: [1, 1], new_value: [2, 3] } },
            { id: 1, mediaId: 1000, mediaName: "A difficult show", payload: { old_value: null, new_value: Status.WATCHING } },
        ]);
    });
