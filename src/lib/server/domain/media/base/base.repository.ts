...
export const createBaseRepository = <TMediaDef extends AnyServerMediaDefinition>(definition: TMediaDef) => {
    const { identity } = definition;
    const repoDefinition: TMediaDef["repository"] = definition.repository;
    const { listTable, mediaTable, tagTable, genreTable } = repoDefinition.tables;
    ...
    return {
        ...
        async bulkInsertUserMedia(rows: TMediaDef["repository"]["tables"]["listTable"]["$inferInsert"][]) {
            const { listTable } = repoDefinition.tables;

            if (rows.length === 0) return [];

            const insertedRows: TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"][] = [];
        ...
        async getTagNames(userId: number) {
            const { tagTable } = repoDefinition.tables;
        ...
        async removeMediaByIds(mediaIds: number[]) {
            const { mediaTable, deleteDependents } = repoDefinition.tables;
        ...
        async getListFilters(userId: number): Promise<ExpandedListFilters> {
            const { tables: { genreTable, tagTable, listTable }, listQuery: { filterOptions } } = repoDefinition;
        ...
        async findById(mediaId: number): Promise<TMediaDef["repository"]["tables"]["mediaTable"]["$inferSelect"] | undefined> {
            const { mediaTable } = repoDefinition.tables;
        ...
        async findByApiId(apiId: number | string): Promise<TMediaDef["repository"]["tables"]["mediaTable"]["$inferSelect"] | undefined> {
            const { mediaTable } = repoDefinition.tables;
        ...
        async updateUserMediaDetails(userId: number, mediaId: number, updateData: TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"]): Promise<TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"]> {
            const { listTable } = repoDefinition.tables;
        ...
        async findUserMedia(userId: number | undefined, mediaId: number): Promise<UserMediaWithTags<TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"]> | null> {
            const { listTable, tagTable } = repoDefinition.tables;
        ...
        async downloadMediaListAsCSV(userId: number): Promise<(TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"] & ExportMediaList)[] | undefined> {
            const { mediaTable, listTable } = repoDefinition.tables;
        ...
        async getUserFollowsMediaData(userId: number | undefined, mediaId: number): Promise<UserFollowsMediaData<TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"]>[]> {
            const { listTable } = repoDefinition.tables;
            ...
            const inFollowsLists = await getDbClient()
                ...
                .innerJoin(userMediaSettings, and(
                    ...
                    eq(userMediaSettings.mediaType, identity.mediaType),
        ...
        async getMediaCommunityActivity(actor: Actor, mediaId: number, search: SearchType) {
            const { tables: { listTable }, communityActivity: { aggregates } } = repoDefinition;
        ...
        async getMediaList(currentUserId: number | undefined, userId: number, args: MediaListArgs): Promise<MediaListData<TMediaDef["repository"]["tables"]["listTable"]["$inferSelect"]>> {
            const { tables: { listTable, mediaTable, tagTable }, listQuery } = repoDefinition;
        ...
        async getTagsView(userId: number, search: SimpleSearch) {
            const { listTable, mediaTable, tagTable } = repoDefinition.tables;
...
export type BaseRepository<TDefinition extends AnyServerMediaDefinition = AnyServerMediaDefinition> = ReturnType<typeof createBaseRepository<TDefinition>>;
