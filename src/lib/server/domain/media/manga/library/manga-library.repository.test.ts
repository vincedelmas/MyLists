...
describe("manga library repository", () => {
    let sqlite: Database;
    ...
    it("hydrates exact historical progress, tags, covers and common items", async () => {
        const result = await new MangaLibraryRepository().getMediaList(50, ownerScope, { page: 1, perPage: 2 });
    ...
    it("uses stable canonical manga IDs for every sort tie", async () => {
        const result = await new MangaLibraryRepository().getMediaList(42, ownerScope, {
            page: 1,
            perPage: 3,
            sorting: "Rating +",
        });
