...
export class MovieAchievementCalculator {
    ...
    static getAchievementCte(achievement: Achievement, userId?: number) {
        const common = this.helper.getCommonAchievementCte(MediaType.MOVIES, achievement, userId);
        ...
        switch (achievement.codeName) {
            ...
            case "animation_movies":
                return this.helper.countCompletedGenre(MediaType.MOVIES, String(achievement.value), userId);
            case "short_movies":
            case "long_movies":
            ...
            default:
                return this.helper.unsupported(MediaType.MOVIES, achievement);
        }
    }
    ...
    private static maxDirector(userId?: number) {
    ...
    private static maxActor(userId?: number) {
        const grouped = getDbClient()
            .select({
                userId: libraryEntry.userId,
                count: count(libraryEntry.catalogItemId).as("count"),
    ...
    private static countLanguages(userId?: number) {
        return getDbClient()
            .select({
                userId: libraryEntry.userId,
                value: countDistinct(movieDetails.originalLanguage).as("value"),
            })
            .from(libraryEntry)
            .innerJoin(catalogItem, eq(catalogItem.id, libraryEntry.catalogItemId))
            .innerJoin(movieDetails, eq(movieDetails.catalogItemId, catalogItem.id))
