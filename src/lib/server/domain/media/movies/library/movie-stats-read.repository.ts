...
/** Movie aggregate and advanced statistics over canonical movie rows. */
export class MovieStatsReadRepository {
    async getAggregatedMediaStats(scope: MovieStatsReadScope) {
    ...
    async getAdvancedMediaStats(scope: MovieStatsReadScope, mediaAvgRating: number | null) {
        const userId = scope.type === "library" ? scope.access.ownerId : undefined;
        const [
            ratings,
            totalTags,
    ...
    private async computeTotalTags(userId?: number) {
        const conditions = [eq(libraryTag.kind, MediaType.MOVIES)];
    ...
    private async computeAverageDuration(userId?: number) {
        return getDbClient()
    ...
    private async computeBudgetRevenue(userId?: number) {
    ...
    private async computeActorAffinity(mediaAvgRating: number | null, userId?: number) {
        const expressions = affinityExpressions(movieActor.name, mediaAvgRating);
        const rows = await getDbClient()
    ...
    private async computeDirectorAffinity(mediaAvgRating: number | null, userId?: number) {
        return this.computeDetailsAffinity(movieDetails.directorName, mediaAvgRating, userId);
    }

    private async computeLanguageAffinity(mediaAvgRating: number | null, userId?: number) {
...
export type MovieStatsReadScope =
