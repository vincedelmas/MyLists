...
const { ProfileHighlightsReadService } = await import("./profile-highlights-read.service");
...
describe("normalized profile highlights", () => {
    ...
    it("hides disabled-family highlights while preserving curated order and custom covers", async () => {
        const result = await new ProfileHighlightsReadService().resolveHighlightedMedia(1);
    ...
    it("allows the owner customization search to find a stored disabled-list entry", async () => {
        await expect(new ProfileHighlightsReadService().searchHighlightedMedia(1, "overview", "Movie"))
            .resolves.toEqual([{
                mediaId: 11,
