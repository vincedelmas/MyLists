...
export class SocialGraphCommandService {
    constructor(
    ...
    async follow(followerId: number, target: { id: number; privacy: PrivacyType }) {
        ...
        return withTransaction(async () => {
            const status = target.privacy === PrivacyType.PRIVATE ? SocialState.REQUESTED : SocialState.ACCEPTED;
            const created = await this.repository.createRelationship(followerId, target.id, status);
            if (!created) return { changed: false, status: (await this.repository.getFollowingStatus(followerId, target.id))?.status };
