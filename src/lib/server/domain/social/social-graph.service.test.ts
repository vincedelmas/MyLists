...
const { SocialGraphReadService } = await import("./social-graph-read.service");
const { SocialGraphCommandService } = await import("./social-graph-command.service");


describe("social graph services", () => {
    let sqlite: Database;
    ...
    it("keeps public header counts separate from access-scoped social members", async () => {
        ...
        const reader = new SocialGraphReadService();
    ...
    it("creates and accepts a private follow request atomically and idempotently", async () => {
        const commands = new SocialGraphCommandService();
