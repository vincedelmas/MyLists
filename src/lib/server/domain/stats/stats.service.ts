...
import type {MediaStatsRegistry} from "@/lib/server/domain/media/media.registries";
...
const getComputedStatsSummary = async (repository: StatsRepository, { userId }: { userId?: number }) => {
    const {
        preComputedStats,
        statusCountsList,
        ...
        mediaBreakdown,
        totalUsers,
    } = await repository.getPreComputedStatsSummary({ userId });
...
export const createStatsService = (
    ...
    return {
        async updateUserMediaListSettings(userId: number, payload: Partial<Record<MediaType, boolean>>) {
            await repository.updateUserMediaListSettings(userId, payload);
        },

        async updateUserPreComputedStatsWithDelta(userId: number, mediaType: MediaType, mediaId: number, delta: DeltaStats) {
            await repository.updateUserPreComputedStatsWithDelta(userId, mediaType, mediaId, delta);
        },

        async updateAllUsersPreComputedStats(mediaType: MediaType, userStats: UserMediaStats[]) {
            await repository.updateAllUsersPreComputedStats(mediaType, userStats);
        },
        ...
        async userHallOfFameData(filters: HallOfFameSearch, userId?: number) {
            const {
                ...
                page, pages, total,
            } = await repository.userHallOfFameData(filters, userId);
        ...
        async userPreComputedStatsSummary(userId: number) {
            const { mediaBreakdown: _mediaBreakdown, ...summary } = await getComputedStatsSummary(repository, { userId });
            return summary;
        },

        async userPerMediaSummaryStats(userId: number) {
            const excludedStatuses = statusUtils.getNoPlanTo();
            const activeSettings = await repository.userActiveMediaSettings(userId);
        ...
        async userAdvancedSummaryStats(userId: number) {
            const [userPreComputedStats, platinumAchievements, updateFingerprint, activityByMonth] = await Promise.all([
                getComputedStatsSummary(repository, { userId }),
                achievementsRepository.countPlatinumAchievements(userId),
                updateHistoryRepository.mediaUpdateFingerprint({ userId }),
                activityService.getActivityStatsByMonth({ userId }),
            ]);
        ...
        async userAdvancedMediaStats(userId: number, mediaType: MediaType) {
            ...
            const preComputedMediaStats = await repository.getAggregatedMediaStats({ userId, mediaType });

            const [activityByMonth, specificMediaStats, updateFingerprint] = await Promise.all([
                activityService.getActivityStatsByMonth({ userId, mediaType }),
                mediaStatistics.calculateAdvancedMediaStats(preComputedMediaStats.avgRated, userId),
                updateHistoryRepository.mediaUpdateFingerprint({ mediaType, userId }),
            ]);
        ...
        async platformAdvancedStatsSummary() {
            const [platformPreComputedStats, platinumAchievements, activityByMonth, updateFingerprint] = await Promise.all([
                getComputedStatsSummary(repository, {}),
                achievementsRepository.countPlatinumAchievements(),
                activityService.getActivityStatsByMonth({ excludeBulkImports: true }),
                updateHistoryRepository.mediaUpdateFingerprint({ excludeBulkImports: true }),
            ]);
        ...
        async platformMediaAdvancedStats(mediaType: MediaType) {
            const mediaStatistics = mediaStatsRegistry.get(mediaType);
            const platformPreComputedStats = await repository.getAggregatedMediaStats({ mediaType });
            ...
            const [specificMediaStats, activityByMonth, updateFingerprint] = await Promise.all([
                ...
                activityService.getActivityStatsByMonth({ mediaType, excludeBulkImports: true }),
                updateHistoryRepository.mediaUpdateFingerprint({ mediaType, excludeBulkImports: true }),
            ]);
