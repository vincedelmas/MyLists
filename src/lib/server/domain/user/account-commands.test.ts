...
const createCapabilities = () => {
    ...
    } as unknown as InactiveAccountCommands;

    return {
        userRepository,
        inactiveAccountService,
        deletion: new AccountDeletionCommands(inactiveAccountService, userRepository),
        settings: new AccountSettingsCommands(userRepository),
    };
};
...
describe("AccountDeletionCommands", () => {
    it("deletes manual accounts after removing inactive account lifecycle rows", async () => {
        const { deletion, userRepository, inactiveAccountService } = createCapabilities();

        await expect(deletion.delete({ type: "manual", userId: 42 })).resolves.toBe(true);
...
describe("AccountSettingsCommands", () => {
    ...
    it("does not hide unrelated database failures", async () => {
        ...
        await expect(settings.update(42, { privacy: "public" }))
